...
const SearchIcon = "ri:search-line";
...
/**
 * 关闭搜索弹窗并重置内部状态
 * @description 通过延时清空关键字与选中项，避免关闭瞬间的视觉闪烁
 */
...
/**
 * 获取当前激活的列表（结果/历史）与路径
 * @returns options 当前列表；currentPath 当前路径；isResultOptions 是否为结果列表
 */
...
/**
 * 拖拽收藏项以重新排序
 * @param item 包含 oldIndex/newIndex 的拖拽信息
 */
...
<template>
  ...
  >
    ...
    >
      <template #prefix>
        <SmartIcon :icon="SearchIcon" class="text-primary w-[24px] h-[24px]" />
      </template>
    </el-input>
