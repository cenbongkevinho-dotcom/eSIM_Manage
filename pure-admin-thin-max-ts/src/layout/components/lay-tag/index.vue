...
const ArrowDown = "ri:arrow-down-s-line";
const ArrowRightSLine = "ri:arrow-right-s-line";
const ArrowLeftSLine = "ri:arrow-left-s-line";
...
/**
 * 引入 i18n t 函数，用于右键菜单中“内容区全屏/退出全屏”文案的国际化
 */
const { t } = useI18n();
...
function onClickDrop(key, item, selectRoute?: RouteConfigs) {
   ...
   * @param key 菜单索引（0-6）
   * @param item 当前菜单项
   * @param selectRoute 右键选中的路由（可选）
   * 说明：
   * - key=6 时切换内容区全屏，同时根据侧边栏隐藏状态动态更新图标与文案（Fullscreen/ExitFullscreen）。
   */
    ...
    case 6:
      ...
      setTimeout(() => {
        if (pureSetting.hiddenSideBar) {
          ...
          tagsViews[6].text = t("common.tagActions.exitContentFullscreen");
        } else {
          tagsViews[6].icon = Fullscreen;
          tagsViews[6].text = t("common.tagActions.contentFullscreen");
        }
      }, 100);
      break;
  }
