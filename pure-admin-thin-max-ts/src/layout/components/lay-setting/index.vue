...
// 设置面板内使用的图标（统一为冒号命名在线模式）
const Check = "ep:check";
const LeftArrow = "ri:arrow-left-s-line";
const RightArrow = "ri:arrow-right-s-line";
...
/**
 * layout-setting 专属 LayPanel 的组件实例引用
 * 用于在路由切换或页面挂载时以编程方式关闭设置面板，
 * 避免在某些场景（如演示页或自动化测试）中被错误地保持为打开状态。
 */
const settingPanelRef = ref<InstanceType<typeof LayPanel> | null>(null);

/**
 * 当前路由对象
 * 我们仅需观察其 fullPath 变化来在每次路由切换时关闭设置面板。
 */
 ...
 * 页签风格选项
 * 说明：
 * - 为示例加入 iconfont 图标演示，使用 IF- 前缀统一走 useRenderIcon（font-class 默认）
 * - 由于 iconfont 库暂仅包含 tabs/logo/new 三个图标，这里统一使用 tabs 图标作演示
 */
...
/**
 * 挂载后兜底关闭一次设置面板，防止在某些热重载/保留状态场景下误开。
 */
onMounted(() => {
  // 函数级注释：确保进入任意页面时，布局设置面板处于关闭态
  settingPanelRef.value?.close?.();
  // 额外保险：主动广播关闭事件，仅针对 layout-setting 通道，避免其它 LayPanel 受影响
  // 这样即便存在遗留的全局事件导致面板处于打开状态，也能在页面挂载后统一关闭。
  emitter.emit("openPanel", { open: false, channel: "layout-setting" });
});

/**
 * 监听路由变化，任意路由切换时都显式关闭布局设置面板。
 * 这样可确保演示页（如 IconAudit）在展示其内部 Panel 交互时不被全局面板遮挡。
 */
watch(
  () => route.fullPath,
  () => {
    // 函数级注释：路由切换回调，统一关闭 layout-setting 面板
    settingPanelRef.value?.close?.();
    // 额外保险：同步广播关闭事件，确保通过事件通道的打开状态被重置
    emitter.emit("openPanel", { open: false, channel: "layout-setting" });
  }
);
</script>
